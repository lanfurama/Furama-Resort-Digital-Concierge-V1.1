...
// Standalone test for Voice Parsing Logic (to bypass environment dependency issues)
...
function convertSpokenToNumber(text: string): string {
    ...
    processed = processed.replace(/([A-Z0-9])\s+([A-Z0-9])/g, "$1$2");
    processed = processed.replace(/([A-Z0-9])\s+([A-Z0-9])/g, "$1$2"); // Run twice for 3+ chars
...
const phoneticCases = [
    ...
    { input: "rùm oan", expected: "room 1", desc: "Phonetic room + number fallback logic check" } // "oan" -> "1" in mapping? Yes.
];
...
testCases.forEach(test => {
    ...
    const isPass = result.includes(test.expected); // Check if expected number is formed
