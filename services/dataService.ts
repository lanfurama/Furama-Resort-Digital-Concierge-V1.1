...
let menuItems: MenuItem[] = [
  // Dining
  {
    ...
    price: 25,
    category: "Dining",
    description: "Premium beef with truffle fries",
    translations: {
      Vietnamese: {
        name: "Burger Bò Wagyu",
        description: "Thịt bò cao cấp kèm khoai tây chiên nấm truffle",
      },
      Korean: {
        name: "와규 버거",
        description: "트러플 감자튀김을 곁들인 프리미엄 소고기",
      },
      Japanese: {
        name: "和牛バーガー",
        description: "トリュフフライ付きのプレミアムビーフ",
      },
      Chinese: { name: "和牛汉堡", description: "优质牛肉配松露薯条" },
      Russian: {
        name: "Вагю Бургер",
        description: "Мраморная говядина с картофелем фри с трюфелем",
      },
    },
  },
  {
    ...
    translations: {
      ...
      Japanese: {
        name: "ベトナムのフォー",
        description: "伝統的な牛肉麺スープ",
      },
      Chinese: { name: "越南河粉", description: "传统牛肉粉" },
      Russian: {
        name: "Вьетнамский Фо",
        description: "Традиционный суп с лапшой и говядиной",
      },
    },
  },
  {
    ...
    price: 18,
    category: "Dining",
    description: "Classic triple-decker with chicken and bacon",
    translations: {
      Vietnamese: {
        name: "Bánh Mì Club",
        description: "Bánh mì kẹp 3 tầng cổ điển với gà và thịt xông khói",
      },
      Korean: {
        name: "클럽 샌드위치",
        description: "치킨과 베이컨이 들어간 클래식 3단 샌드위치",
      },
      Japanese: {
        name: "クラブサンドイッチ",
        description: "チキンとベーコンのクラシックな3段重ね",
      },
      Chinese: { name: "公司三明治", description: "经典三层鸡肉培根三明治" },
      Russian: {
        name: "Клубный Сэндвич",
        description: "Классический трехслойный сэндвич с курицей и беконом",
      },
    },
  },
  // Spa
  {
    ...
    price: 80,
    category: "Spa",
    description: "60 mins relaxing massage",
    translations: {
      Vietnamese: { name: "Massage Tinh Dầu", description: "60 phút thư giãn" },
      Korean: {
        name: "아로마테라피 마사지",
        description: "60분 릴렉싱 마사지",
      },
      Japanese: {
        name: "アロマテラピーマッサージ",
        description: "60分間のリラックスマッサージ",
      },
      Chinese: { name: "香薰按摩", description: "60分钟放松按摩" },
      Russian: {
        name: "Ароматерапевтический Массаж",
        description: "60 минут расслабляющего массажа",
      },
    },
  },
  {
    ...
    price: 95,
    category: "Spa",
    description: "75 mins deep tissue relief",
  },
  {
    id: "6",
    ...
    price: 60,
    category: "Spa",
    description: "45 mins organic facial",
  },
  // Pool
  {
    id: "7",
    ...
    price: 5,
    category: "Pool",
    description: "Chilled fresh coconut",
  },
  {
    id: "8",
  ...
  {
    id: "9",
    name: "Pool Towel Request",
    price: 0,
    category: "Pool",
    description: "Extra large resort towel",
  },
  // Butler
  {
    id: "10",
    ...
    price: 0,
    category: "Butler",
    description: "Bucket of ice cubes",
  },
  {
    id: "11",
    ...
    price: 0,
    category: "Butler",
    description: "Complimentary shoe polishing",
  },
  {
    id: "12",
    ...
    price: 0,
    category: "Butler",
    description: "Assistance with unpacking luggage",
  },
];
...
let events: ResortEvent[] = [
  ...
  },
  {
    ...
    date: "2023-11-20",
    time: "18:30",
    location: "Cafe Indochine",
    description: "All-you-can-eat fresh local seafood.",
  },
];
...
let serviceRequests: ServiceRequest[] = [
  {
    ...
    type: "DINING",
    status: "COMPLETED",
    details: "Order for: Wagyu Burger, Club Sandwich",
    roomNumber: "101",
    timestamp: Date.now() - 86400000,
    confirmedAt: Date.now() - 85000000,
    completedAt: Date.now() - 83000000,
  },
  {
    id: "h2",
    type: "SPA",
    status: "COMPLETED",
    details: "Order for: Aromatherapy Massage",
    roomNumber: "205",
    timestamp: Date.now() - 172800000,
    confirmedAt: Date.now() - 171800000,
    completedAt: Date.now() - 169000000,
  },
  {
    id: "h3",
    type: "BUTLER",
    status: "COMPLETED",
    details: "Order for: Ice Bucket",
    roomNumber: "101",
  ...
  },
  {
    ...
    type: "POOL",
    status: "PENDING",
    details: "Order for: Fresh Coconut",
    roomNumber: "888",
...
let notifications: AppNotification[] = [
  {
    ...
    recipientId: "101",
    title: "Welcome",
    message: "Welcome to Furama Resort & Villas!",
    type: "INFO",
...
export const getNotifications = async (
  ...
  try {
    const dbNotifications = await apiClient.get<any[]>(
...
const notifyStaffByDepartment = async (
  department: string,
  title: string,
  message: string,
) => {
  ...
  try {
    ...
    const mapped = dbUsers.map((user) => ({
      ...
      checkIn: user.check_in
        ? typeof user.check_in === "string"
          ? user.check_in
          : new Date(user.check_in).toISOString()
        : undefined,
      checkOut: user.check_out
        ? typeof user.check_out === "string"
          ? user.check_out
          : new Date(user.check_out).toISOString()
        : undefined,
...
export const addUser = async (user: User): Promise<void> => {
  try {
    ...
    users = [
      ...users,
      { ...user, id: Date.now().toString(), password: user.password || "123" },
    ];
    throw error;
  }
};
...
export const updateUser = async (
  ...
  try {
    ...
    if (!dbUser) {
      ...
      console.log(
        "User updated in local cache (from request body):",
        updatedUser,
      );
      return updatedUser;
    }
...
export const generateCheckInCode = async (
  userId: string,
): Promise<{ checkInCode: string; user: User }> => {
  try {
    const response = await apiClient.post<{
      success: boolean;
      checkInCode: string;
      user: any;
    }>(`/users/${userId}/generate-check-in-code`, {});
    if (response.success && response.checkInCode) {
      ...
      const frontendUser: User = {
        ...
        checkIn: response.user.check_in
          ? new Date(response.user.check_in).toISOString()
          : undefined,
        checkOut: response.user.check_out
...
export const updateUserNotes = async (
  ...
  try {
    ...
    if (!user || !user.id) {
      ...
      console.error(
        "Failed to update user notes - User not found:",
        roomNumber,
      );
      throw error;
    }
...
export const updateDriverHeartbeat = async (userId: string): Promise<void> => {
  try {
    ...
    console.log(
      "[Heartbeat] Heartbeat successful, updated_at:",
      result.updated_at,
    );
...
export const setDriverOnlineFor10Hours = async (
  userId: string,
): Promise<void> => {
  try {
    console.log(
      "[First Login] Setting driver online for 10 hours, userId:",
      userId,
    );
    console.log(
      "[First Login] Calling API: POST /users/" + userId + "/online-10hours",
    );
    const response = await apiClient.post(
      `/users/${userId}/online-10hours`,
      {},
    );
    console.log("[First Login] API response:", response);
    console.log(
      "[First Login] Driver set online for 10 hours successfully, updated_at:",
      response?.updated_at,
    );
  } catch (error: any) {
    console.error(
      "[First Login] Failed to set driver online for 10 hours:",
      error,
    );
...
export const updateUserLanguage = async (
  ...
  try {
    ...
    if (updatedUser.language !== language) {
      console.warn(
        "WARNING: Language mismatch! Expected:",
        language,
        "Got:",
...
export const loginGuest = (
  ...
  if (!user) {
    return {
      success: false,
      message: "Guest not found. Please check Room # and Last Name.",
    };
  }
...
export const importGuestsFromCSV = async (
  csvContent: string,
): Promise<number> => {
    ...
    if (parts.length >= 2) {
      ...
      if (lastName && roomNumber) {
        try {
          ...
          count++;
          console.log(
            `Guest "${lastName}" (${roomNumber}) imported successfully`,
          );
        } catch (error: any) {
          console.error(
            `Failed to import guest "${lastName}" (${roomNumber}):`,
            error,
          );
          errors.push(
            `${lastName} (${roomNumber}): ${error?.message || "Unknown error"}`,
          );
  ...
  console.log(
    `CSV import completed: ${count} guests imported${errors.length > 0 ? `, ${errors.length} failed` : ""}`,
  );
...
export const updateLocation = async (
  ...
  try {
    ...
    const dbLocation = await apiClient.put<any>(
      `/locations/${id}`,
      requestBody,
    );
...
export const deleteLocation = async (idOrName: string): Promise<void> => {
  try {
    console.log(
      "[deleteLocation] Attempting to delete location with id/name:",
      idOrName,
    );
    console.log(
      "[deleteLocation] Current locations cache:",
      locations.map((l) => ({ id: l.id, name: l.name, idType: typeof l.id })),
    );
    ...
    if (location && location.id) {
      ...
      console.log(
        "[deleteLocation] API delete successful, response:",
        deleteResult,
      );
      ...
      console.log(
        `[deleteLocation] Local cache updated: ${beforeCount} -> ${afterCount} locations`,
      );
    } else {
      console.warn(
        "[deleteLocation] Location not found in cache, trying to delete directly via API",
      );
      ...
      try {
        ...
        console.log(
          "[deleteLocation] API delete successful (direct delete), response:",
          deleteResult,
        );
        ...
        try {
          ...
          console.error(
            "[deleteLocation] Failed to refresh locations after delete:",
            refreshError,
          );
          ...
          const afterCount = locations.length;
          console.log(
            `[deleteLocation] Fallback local delete: ${beforeCount} -> ${afterCount} locations`,
          );
        }
      } catch (apiError) {
        ...
        const afterCount = locations.length;
        console.log(
          `[deleteLocation] Final fallback local delete: ${beforeCount} -> ${afterCount} locations`,
        );
        if (beforeCount === afterCount) {
          console.warn(
            "[deleteLocation] No location was deleted! Location may not exist.",
          );
        }
        throw apiError;
      }
...
export const addRoomType = async (rt: RoomType): Promise<RoomType> => {
  try {
    ...
    console.warn(
      "Falling back to local mock data. Room type will NOT be saved to database!",
    );
    // Fallback to local
...
export const updateRoomType = async (
  ...
  try {
    ...
    const dbRoomType = await apiClient.put<any>(
      `/room-types/${id}`,
      requestBody,
    );
...
export const importRoomsFromCSV = async (
  csvContent: string,
): Promise<number> => {
...
export const getMenu = async (
  categoryFilter?: "Dining" | "Spa" | "Pool" | "Butler",
): Promise<MenuItem[]> => {
  try {
    ...
    console.warn(
      "Falling back to mock menu data. This means menu items are NOT saved to database!",
    );
...
export const addMenuItem = async (item: MenuItem): Promise<void> => {
  try {
    ...
    console.warn(
      "Falling back to local mock data. Menu item will NOT be saved to database!",
    );
    // Fallback to local state
...
export const deleteMenuItem = async (id: string): Promise<void> => {
  try {
    ...
    console.warn(
      "Falling back to local mock data. Menu item will NOT be deleted from database!",
    );
    // Fallback to local state
    menuItems = menuItems.filter((m) => m.id !== id);
    throw error; // Re-throw để component có thể handle error
...
export const getEvents = async (): Promise<ResortEvent[]> => {
  try {
    ...
    console.warn(
      "Falling back to mock events. This means events are NOT loaded from database!",
    );
...
export const getPromotions = async (): Promise<Promotion[]> => {
  try {
    ...
    console.warn(
      "Falling back to mock promotions. This means promotions are NOT loaded from database!",
    );
...
export const addPromotion = async (promo: Promotion): Promise<void> => {
  try {
    ...
    console.warn(
      "Falling back to local mock data. Promotion will NOT be saved to database!",
    );
    // Fallback to local state
    promotions = [
      ...promotions,
      { ...promo, id: Date.now().toString(), imageColor: "bg-emerald-500" },
    ];
    throw error;
  }
};
...
export const updatePromotion = async (
  ...
  try {
    ...
    if (promo.description !== undefined)
      requestBody.description = promo.description || null;
    requestBody.language = language;

    // Shared fields (will be updated for all languages)
    if (promo.discount !== undefined)
      requestBody.discount = promo.discount || null;
    if (promo.validUntil !== undefined)
      requestBody.valid_until = promo.validUntil || null;
    if (promo.imageColor !== undefined)
      requestBody.image_color = promo.imageColor || "bg-emerald-500";
    if (promo.imageUrl !== undefined)
      requestBody.image_url = promo.imageUrl || null;

    console.log("Updating promotion via API - ID:", id);
    console.log("Updating promotion via API - Request Body:", requestBody);
    console.log(
      "Note: Shared fields (discount, valid_until, image_color, image_url) will be updated for all languages",
    );

    const dbPromotion = await apiClient.put<any>(
      `/promotions/${id}`,
      requestBody,
    );
    ...
    try {
      ...
      console.warn(
        "Failed to refresh promotions, using single update result:",
        refreshError,
      );
    }
...
export const deletePromotion = async (id: string): Promise<void> => {
  try {
    ...
    console.warn(
      "Falling back to local mock data. Promotion will NOT be deleted from database!",
    );
    // Fallback to local state
    promotions = promotions.filter((p) => p.id !== id);
    throw error; // Re-throw để component có thể handle error
...
export const getKnowledgeBase = async (): Promise<KnowledgeItem[]> => {
  try {
    ...
    console.warn(
      "Falling back to mock knowledge. This means knowledge items are NOT loaded from database!",
    );
  ...
  try {
    ...
    const mapped = dbRides.map((r) => {
      ...
      if (r.timestamp && typeof r.timestamp === "number" && r.timestamp > 0) {
        ...
        reqTimestamp = isNaN(createdDate.getTime())
          ? Date.now()
          : createdDate.getTime();
      } else {
        // Fallback: use Date.now() if both are missing
        console.warn(
          `[getRides] Missing timestamp for ride ${r.id}, using Date.now()`,
    ...
    console.warn(
      "Falling back to mock rides. This means rides are NOT loaded from database!",
    );
...
export const createManualRide = async (
  ...
  try {
    ...
    const guest = allUsers.find(
      (u) => u.roomNumber === roomNumber && u.role === UserRole.GUEST,
    );
    ...
    const mappedRide: RideRequest = {
      ...
      timestamp:
        dbRide.timestamp ||
        (dbRide.created_at
          ? new Date(dbRide.created_at).getTime()
          : Date.now()),
...
export const getRideById = async (
  ...
  try {
    ...
    if (
      dbRide.timestamp &&
      typeof dbRide.timestamp === "number" &&
      dbRide.timestamp > 0
    ) {
    ...
    return {
      ...
      pickedUpAt: dbRide.pick_timestamp
        ? new Date(dbRide.pick_timestamp).getTime()
        : dbRide.picked_up_at
          ? new Date(dbRide.picked_up_at).getTime()
          : undefined,
      completedAt: dbRide.drop_timestamp
        ? new Date(dbRide.drop_timestamp).getTime()
        : dbRide.completed_at
          ? new Date(dbRide.completed_at).getTime()
          : undefined,
      confirmedAt: dbRide.assigned_timestamp
        ? new Date(dbRide.assigned_timestamp).getTime()
        : dbRide.assigned_at
          ? new Date(dbRide.assigned_at).getTime()
          : undefined,
...
export const updateRideStatus = async (
  ...
  try {
    ...
    if (!ride) {
      console.warn(
        `Ride ${rideId} not found in local cache, but will attempt API update anyway`,
      );
    }
    ...
    const updatedRide: RideRequest = {
      ...
      driverId: dbRide.driver_id
        ? dbRide.driver_id.toString()
        : driverId || ride?.driverId,
      eta:
        dbRide.eta !== undefined
          ? dbRide.eta
          : eta !== undefined
            ? eta
            : ride?.eta,
      pickedUpAt: dbRide.pick_timestamp
        ...
        : dbRide.picked_up_at
          ? new Date(dbRide.picked_up_at).getTime()
          : status === BuggyStatus.ON_TRIP && !ride?.pickedUpAt
            ? Date.now()
            : ride?.pickedUpAt,
      completedAt: dbRide.drop_timestamp
        ? new Date(dbRide.drop_timestamp).getTime()
        : dbRide.completed_at
          ? new Date(dbRide.completed_at).getTime()
          : status === BuggyStatus.COMPLETED && !ride?.completedAt
            ? Date.now()
            : ride?.completedAt,
      confirmedAt: dbRide.assigned_timestamp
        ? new Date(dbRide.assigned_timestamp).getTime()
        : dbRide.assigned_at
          ? new Date(dbRide.assigned_at).getTime()
          : status === BuggyStatus.ASSIGNED && !ride?.confirmedAt
            ? Date.now()
            : status === BuggyStatus.ASSIGNED && ride?.confirmedAt
              ? ride.confirmedAt
    ...
    if (roomNumber) {
      if (status === BuggyStatus.ASSIGNED) {
        sendNotification(
          roomNumber,
          "Buggy Assigned",
          `A driver is on the way. ETA: ${eta} mins.`,
          "SUCCESS",
        );
        ...
        sendNotification(
          roomNumber,
          "Buggy Arriving",
          `Your buggy has arrived at ${updatedRide.pickup}.`,
          "INFO",
        );
        ...
        sendNotification(
          roomNumber,
          "Ride Completed",
          `You have arrived at ${updatedRide.destination}. Have a nice day!`,
          "SUCCESS",
        );
      }
    }
    ...
    console.warn(
      "Falling back to local mock data. Ride status will NOT be updated in database!",
    );
    // Fallback to local state
    const ride = rides.find((r) => r.id === rideId);
    if (!ride) return;
    ...
    rides = rides.map((r) => {
      if (r.id === rideId) {
        ...
        if (
          (status === BuggyStatus.ASSIGNED ||
            status === BuggyStatus.ARRIVING) &&
          !updatedRide.confirmedAt
        ) {
    ...
    if (status === BuggyStatus.ASSIGNED) {
      ...
      sendNotification(
        ride.roomNumber,
        "Buggy Arriving",
        `Your buggy has arrived at ${ride.pickup}.`,
        "INFO",
      );
    } else if (status === BuggyStatus.COMPLETED) {
      sendNotification(
        ride.roomNumber,
        "Ride Completed",
        `You have arrived at ${ride.destination}. Have a nice day!`,
        "SUCCESS",
      );
    }
    throw error; // Re-throw để component có thể handle error
...
export const updateRide = async (
  ride: Partial<RideRequest> & { id: string },
): Promise<RideRequest> => {
  try {
    const { id, ...updateData } = ride;

    const requestBody: any = {};
    if (updateData.guestName !== undefined)
      requestBody.guest_name = updateData.guestName;
    if (updateData.roomNumber !== undefined)
      requestBody.room_number = updateData.roomNumber;
    if (updateData.pickup !== undefined) requestBody.pickup = updateData.pickup;
    if (updateData.destination !== undefined)
      requestBody.destination = updateData.destination;
    if (updateData.status !== undefined) requestBody.status = updateData.status;
    if (updateData.driverId !== undefined)
      requestBody.driver_id = updateData.driverId;
    if (updateData.eta !== undefined) requestBody.eta = updateData.eta;
    if (updateData.guestCount !== undefined)
      requestBody.guest_count = updateData.guestCount;
    if (updateData.notes !== undefined) requestBody.notes = updateData.notes;
    if (updateData.timestamp !== undefined)
      requestBody.timestamp = updateData.timestamp;
    if (updateData.isMerged !== undefined)
    ...
    console.log("Ride updated successfully - API Response:", dbRide);
    ...
    const updatedRide: RideRequest = {
      ...
      driverId: dbRide.driver_id ? dbRide.driver_id.toString() : undefined,
      eta: dbRide.eta,
      guestCount: dbRide.guest_count,
      notes: dbRide.notes,
      pickedUpAt: dbRide.picked_up_at,
      completedAt: dbRide.completed_at,
      confirmedAt: dbRide.confirmed_at,
      rating: dbRide.rating,
      feedback: dbRide.feedback,
    };

    const existingIndex = rides.findIndex((r) => r.id === id);
    if (existingIndex >= 0) {
      rides[existingIndex] = { ...rides[existingIndex], ...updatedRide };
    } else {
      rides = [updatedRide, ...rides];
    }
    ...
    console.error(`Failed to update ride ${ride.id}:`, error);
    throw error;
  }
};
...
export const getActiveRideForUser = async (
  roomNumber: string,
): Promise<RideRequest | undefined> => {
  try {
    // Try to get active ride - API returns 404 if no active ride, which is OK
    const dbRide = await apiClient
      .get<any>(`/ride-requests/room/${roomNumber}/active`)
      .catch((error: any) => {
    ...
    return {
      ...
      confirmedAt: dbRide.assigned_timestamp
        ...
        : dbRide.assigned_at
          ? new Date(dbRide.assigned_at).getTime()
          : dbRide.status === "ASSIGNED" && dbRide.updated_at
            ? new Date(dbRide.updated_at).getTime()
            : undefined,
      pickedUpAt: dbRide.pick_timestamp
        ? new Date(dbRide.pick_timestamp).getTime()
        : dbRide.picked_up_at
    ...
    // Fallback to local
    const localRide = rides.find(
      (r) => r.roomNumber === roomNumber && r.status !== BuggyStatus.COMPLETED,
    );
    // Ensure confirmedAt is set for local rides if status is ASSIGNED/ARRIVING
    if (
      localRide &&
      (localRide.status === BuggyStatus.ASSIGNED ||
        localRide.status === BuggyStatus.ARRIVING) &&
      !localRide.confirmedAt
    ) {
  ...
  try {
    ...
    try {
      ...
      const buggyReqs: ServiceRequest[] = dbRideRequests.map((r) => ({
        ...
        confirmedAt: r.assigned_at
          ? new Date(r.assigned_at).getTime()
          : r.status === "ASSIGNED" && r.updated_at
        ...
        pickedUpAt: r.picked_up_at
          ? new Date(r.picked_up_at).getTime()
          : r.status === "ON_TRIP" && r.updated_at
            ? new Date(r.updated_at).getTime()
            : undefined,
        arrivingAt: r.arriving_at
          ? new Date(r.arriving_at).getTime()
          : r.status === "ARRIVING" && r.updated_at
            ? new Date(r.updated_at).getTime()
            : undefined,
        completedAt: r.completed_at
          ? new Date(r.completed_at).getTime()
          : r.status === "COMPLETED" && r.updated_at
            ? new Date(r.updated_at).getTime()
            : undefined,
        rating: r.rating || undefined,
        feedback: r.feedback || undefined,
...
export const addServiceRequest = async (
  ...
  try {
    ...
    const dbRequest = await apiClient.post<any>(
      "/service-requests",
      requestBody,
    );

    console.log(
      "Service request added successfully - API Response:",
      dbRequest,
    );
...
export const updateServiceStatus = async (
  ...
  try {
    ...
    console.log(
      "Updating service request status via API - Update Data:",
      updateData,
    );

    const dbRequest = await apiClient.put<any>(
      `/service-requests/${id}`,
      updateData,
    );

    console.log(
      "Service request status updated successfully - API Response:",
      dbRequest,
    );
    ...
    const updatedRequest: ServiceRequest = {
      ...
      timestamp:
        ...
        (dbRequest.created_at
          ? new Date(dbRequest.created_at).getTime()
          : Date.now()),
      confirmedAt: dbRequest.confirmed_at
        ? new Date(dbRequest.confirmed_at).getTime()
        : req?.confirmedAt || undefined,
      completedAt: dbRequest.completed_at
        ? new Date(dbRequest.completed_at).getTime()
    ...
    if (status === "CONFIRMED") {
      sendNotification(
        roomNumber,
        "Order Confirmed",
        `Your ${requestType} request has been confirmed.`,
        "SUCCESS",
      );
      ...
      sendNotification(
        roomNumber,
        "Order Completed",
        `Your ${requestType} service is complete.`,
        "SUCCESS",
      );
    }
  } catch (error: any) {
    ...
    console.warn(
      "Falling back to local mock data. Service request status will NOT be updated in database!",
    );
    // Fallback to local state
    const req = serviceRequests.find((s) => s.id === id);
    if (!req) return;
    ...
    serviceRequests = serviceRequests.map((s) => {
      if (s.id === id) {
        ...
        if (status === "CONFIRMED" && !s.confirmedAt)
    ...
    if (status === "CONFIRMED") {
      sendNotification(
        req.roomNumber,
        "Order Confirmed",
        `Your ${req.type.toLowerCase()} request has been confirmed.`,
        "SUCCESS",
      );
    } else if (status === "COMPLETED") {
      sendNotification(
        req.roomNumber,
        "Order Completed",
        `Your ${req.type.toLowerCase()} service is complete.`,
        "SUCCESS",
      );
    }
    throw error; // Re-throw để component có thể handle error
  ...
  try {
    ...
    if (isRideRequest) {
      ...
      try {
        ...
        const updatedRide = await apiClient.put(`/ride-requests/${id}`, {
          rating,
          feedback,
        });
      ...
      try {
        ...
        const updatedRequest = await apiClient.put(`/service-requests/${id}`, {
          rating,
          feedback,
        });
        ...
        try {
          ...
          throw new Error(
            `Failed to rate request: ${serviceError.message || rideError.message}`,
          );
        }
      }
    }
...
// --- HOTEL REVIEWS ---
export const submitHotelReview = async (
  review: HotelReview,
  ...
  try {
    ...
    if (existingReviewId) {
      ...
      dbReview = await apiClient.put<any>(
        `/hotel-reviews/${existingReviewId}`,
        requestBody,
      );
    ...
    const mappedReview: HotelReview = {
      ...
      categoryRatings:
        typeof dbReview.category_ratings === "string"
    ...
    hotelReviews = hotelReviews.filter(
      (r) => r.roomNumber !== review.roomNumber,
    );
...
export const getHotelReview = async (
  roomNumber: string,
): Promise<HotelReview | undefined> => {
  try {
    console.log("Fetching hotel review from API for room:", roomNumber);
    const dbReview = await apiClient
      .get<any>(`/hotel-reviews/room/${roomNumber}`)
      .catch(() => null);
...
const mapServiceRequestToFrontend = (dbReq: any): ServiceRequest => {
  ...
  try {
    if (dbReq.items) {
      items =
        typeof dbReq.items === "string" ? JSON.parse(dbReq.items) : dbReq.items;
    }
  } catch (e) {
  ...
  return {
    ...
    timestamp: dbReq.created_at
      ? new Date(dbReq.created_at).getTime()
      : Date.now(),
    rating: dbReq.rating || undefined,
    feedback: dbReq.feedback || undefined,
    confirmedAt: dbReq.confirmed_at
      ? new Date(dbReq.confirmed_at).getTime()
      : undefined,
    completedAt: dbReq.completed_at
      ? new Date(dbReq.completed_at).getTime()
      : undefined,
  };
};
...
export const getUnifiedHistory = async (): Promise<ServiceRequest[]> => {
  try {
    ...
    const apiServiceRequests = dbServiceRequests.map(
      mapServiceRequestToFrontend,
    );
...
export const getActiveGuestOrders = async (
  roomNumber: string,
): Promise<ServiceRequest[]> => {
  try {
...
export const getCompletedGuestOrders = async (
  roomNumber: string,
): Promise<ServiceRequest[]> => {
  try {
    const all = await getGuestHistory(roomNumber);
...
export const getServiceMessages = async (
  ...
  try {
    ...
    const dbMessages = await apiClient.get<any[]>(
      `/chat-messages/room/${roomNumber}?service_type=${service}`,
    );
...
export const getServiceMessagesSync = (
  roomNumber: string,
  service: string,
): ChatMessage[] => {
  const key = `${roomNumber}_${service}`;
...
export const getServiceUnreadCount = async (
  roomNumber: string,
  service: string,
  userRole: "user" | "staff",
): Promise<number> => {
  try {
    const response = await apiClient.get<{ unreadCount: number }>(
      `/chat-messages/room/${roomNumber}/unread?service_type=${service}&user_role=${userRole}`,
    );
...
export const sendServiceMessage = async (
  ...
  try {
    ...
    serviceChatsCache[key] = [
      ...serviceChatsCache[key],
      mapChatMessageToFrontend(dbMessage),
    ];
    ...
    if (senderRole === "user") {
      setTimeout(async () => {
        ...
        try {
          ...
          serviceChatsCache[key] = [
            ...serviceChatsCache[key],
            mapChatMessageToFrontend(staffMessage),
          ];
        } catch (error) {
    ...
    serviceChats[key] = [
      ...serviceChats[key],
      { id: Date.now().toString(), role: senderRole, text },
    ];
  }
};
...
export const getDashboardStats = async () => {
  ...
  const pendingDining = allRequests.filter(
    (r) => r.type === "DINING" && r.status === "PENDING",
  ).length;
  const pendingSpa = allRequests.filter(
    (r) => r.type === "SPA" && r.status === "PENDING",
  ).length;
  const pendingPool = allRequests.filter(
    (r) => r.type === "POOL" && r.status === "PENDING",
  ).length;
  const pendingButler = allRequests.filter(
    (r) => r.type === "BUTLER" && r.status === "PENDING",
  ).length;

  // 3. Buggies
  const allRides = await getRides().catch(() => getRidesSync());
  const activeBuggies = allRides.filter(
    (r) =>
      r.status === BuggyStatus.ON_TRIP ||
      r.status === BuggyStatus.ARRIVING ||
      r.status === BuggyStatus.ASSIGNED,
  ).length;
  const searchingBuggies = allRides.filter(
    (r) => r.status === BuggyStatus.SEARCHING,
  ).length;
  const onTripBuggies = allRides.filter(
    (r) =>
      r.status === BuggyStatus.ON_TRIP ||
      r.status === BuggyStatus.ARRIVING ||
      r.status === BuggyStatus.ASSIGNED,
  ...
  const completedBuggies = allRides.filter(
    (r) => r.status === BuggyStatus.COMPLETED,
  ).length;
  ...
  const todayCompleted = allRequests.filter(
    (r) => r.status === "COMPLETED" && r.timestamp > todayStart.getTime(),
  );
  const diningRev =
    todayCompleted.filter((r) => r.type === "DINING").length * 40;
  ...
  const recentActivity = (await getUnifiedHistory().catch(() => []))
...
export const getDriverPerformanceStats = async (
  ...
  try {
    ...
    const periodRides = allRides.filter(
      (r) =>
    ...
    const driverStatsArray = Array.from(driverStatsMap.values()).map(
      (stats) => {
        // Calculate averages
        stats.avgRating =
        ...
        stats.avgResponseTime =
          stats.responseTimeCount > 0
            ...
            : 0;
        stats.avgTripTime =
          stats.tripTimeCount > 0
            ? stats.totalTripTime / stats.tripTimeCount
            : 0;
